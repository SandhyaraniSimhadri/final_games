	...
	.then(availableCacheNames =>
	{
		...
		return Promise.resolve().then(() =>
		{
			...
			return clients.matchAll().then(clients =>
			{
				...
				// verify client count is as expected in navigate requests.
				// find a way to upgrade on reloading the only client. Chrome seems to think there are 2 clients in that case.
				if (clients.length > 1)
					return availableCacheNames[0];
